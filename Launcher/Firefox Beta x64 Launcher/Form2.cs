...
namespace Firefox_Beta_x64_Launcher
{
    public partial class Form2 : Form
    {
        ...
        private void Button1_Click(object sender, System.EventArgs e)
        {
            ...
            {
                File.WriteAllText(@"Firefox Beta x64\updates\Profile.txt", "-allow-downgrade -no-remote -profile \"profile\"");
                this.Close();
            }
            if (radioButton2.Checked)
            {
                File.WriteAllText(@"Firefox Beta x64\updates\Profile.txt", "-no-remote -profile \"Firefox Beta x64\\profile\"");
                this.Close();
            }
            if (radioButton3.Checked)
            {
                File.WriteAllText(@"Firefox Beta x64\updates\Profile.txt", "");
                this.Close();
            }
        }
