...
namespace Firefox_Beta_x86_Launcher
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(@"Firefox Beta x86\Firefox.exe"))
            {
                if (!File.Exists(@"Firefox Beta x86\updates\Profile.txt"))
                {
                    if (culture1.Name == "de-DE")
                    {
                        ...
                        String Arguments = File.ReadAllText(@"Firefox Beta x86\updates\Profile.txt");
            ...
            {
                string message = "Firefox Beta x86 ist nicht installiert";
                MessageBox.Show(message, "Firefox Beta x86 Launcher", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else
            {
                string message = "Firefox Beta x86 is not installed";
                MessageBox.Show(message, "Firefox Beta x86 Launcher", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
    }
