...
namespace Firefox_Nightly_x86_Launcher
{
    public partial class Form1 : Form 
    {
        ...
        private void Button1_Click(object sender, System.EventArgs e)
        {
            ...
            {
                File.WriteAllText(@"Firefox Nightly x86\updates\Profile.txt", "-allow-downgrade -no-remote -profile \"profile\"");
                this.Close();
            }
            if (radioButton2.Checked)
            {
                File.WriteAllText(@"Firefox Nightly x86\updates\Profile.txt", "-no-remote -profile \"Firefox Nightly x86\\profile\"");
                this.Close();
            }
            if (radioButton3.Checked)
            {
                File.WriteAllText(@"Firefox Nightly x86\updates\Profile.txt", "");
                this.Close();
            }
        }
