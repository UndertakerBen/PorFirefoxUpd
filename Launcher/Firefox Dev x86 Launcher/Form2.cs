...
namespace Firefox_Dev_x86_Launcher
{
    public partial class Form2 : Form
    {
        ...
        private void Button1_Click(object sender, System.EventArgs e)
        {
            ...
            {
                File.WriteAllText(@"Firefox Dev x86\updates\Profile.txt", "-allow-downgrade -no-remote -profile \"profile\"");
                this.Close();
            }
            if (radioButton2.Checked)
            {
                File.WriteAllText(@"Firefox Dev x86\updates\Profile.txt", "-no-remote -profile \"Firefox Dev x86\\profile\"");
                this.Close();
            }
            if (radioButton3.Checked)
            {
                File.WriteAllText(@"Firefox Dev x86\updates\Profile.txt", "");
                this.Close();
            }
        }
