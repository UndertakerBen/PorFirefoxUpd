...
namespace Firefox_ESR_x64_Launcher
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(applicationPath + "\\Firefox ESR x64\\Firefox.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Firefox ESR x64\\updates\\Profile.txt"))
                {
                    Application.EnableVisualStyles();
                    Application.SetCompatibleTextRenderingDefault(false);
                    Application.Run(new Form1());
                    String Arguments = File.ReadAllText(applicationPath + "\\Firefox ESR x64\\updates\\Profile.txt") + sb.ToString();
                    if (Arguments.Contains("-profile \"Firefox"))
                    {
                        ...
                        Process.Start(applicationPath + "\\Firefox ESR x64\\Firefox.exe", Arguments3);
                    }
                    else
                    {
                        Process.Start(applicationPath + "\\Firefox ESR x64\\Firefox.exe", Arguments);
                    }
                }
                else
                {
                    String Arguments = File.ReadAllText(applicationPath + "\\Firefox ESR x64\\updates\\Profile.txt") + sb.ToString();
                    if (File.Exists(applicationPath + "\\Firefox ESR x64\\profile\\extensions.json"))
                    {
                        File.Delete(applicationPath + "\\Firefox ESR x64\\profile\\extensions.json");
                        if (Arguments.Contains("-profile \"Firefox"))
                        {
                            string[] Arguments2 = Arguments.Split(new char[] { '"' }, 3);
                            string Arguments3 = Arguments2[0].Replace("-no-remote ", "") + "\"" + applicationPath + "\\" + Arguments2[1] + "\"" + Arguments2[2];
